...
import './appsco-access-report-account-item.js';
...
class AppscoAccessReportAccounts extends mixinBehaviors([AppscoListBehavior], PolymerElement) {
  static get template() {
    return html`
            ...
            <template is="dom-if" if="[[ !_listEmpty ]]">
                ...
                <div class="list">
                    <template is="dom-repeat" items="[[ _listItems ]]" on-dom-change="_onItemsDomChange">
                        ...
                        <appsco-access-report-account-item id="appscoListItem_[[ index ]]" item="[[ item ]]" type="[[ type ]]" authorization-token="[[ authorizationToken ]]" number-of-resources-to-display="[[ numberOfResourcesToDisplay ]]" load-more="[[ loadMore ]]"></appsco-access-report-account-item>
                    </template>
                </div>
            </template>
        </div>
...
`;
  ...
  static get is() { return 'appsco-access-report-accounts'; }
...
window.customElements.define(AppscoAccessReportAccounts.is, AppscoAccessReportAccounts);
