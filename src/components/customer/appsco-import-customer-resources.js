...
class AppscoImportCustomerResources extends PolymerElement {
  static get template() {
    return html`
        <style>
            :host {
                display: block;
                position: relative;
                @apply --appsco-import-customer-resources;
            }
            :host .info {
                margin: 0;
            }
        ...
        <appsco-import-resource id="appscoImportResource" authorization-token="[[ authorizationToken ]]" import-api="[[ importApi ]]">
            ...
            <h2 title="" slot="title">Import CSV file</h2>

            <div info="" slot="info">
                <p class="info">
                    To get started, prepare your organization’s business customer resources data so it’s in the proper format
                    for importing to AppsCo. Allowed format is csv.
                </p><ul>
                    <li>
                        If you are importing more then 500 resources at a time, split them into multiple
...
`;
  ...
  static get is() { return 'appsco-import-customer-resources'; }
  ...
  _onDownloadExampleImportFile(event) {
      ...
      window.location.href = this.domain + '/example_customer_resource_import.csv';
  }
}
window.customElements.define(AppscoImportCustomerResources.is, AppscoImportCustomerResources);
