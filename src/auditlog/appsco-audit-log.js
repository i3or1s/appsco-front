...
class AppscoAuditLog extends mixinBehaviors([
    AppLocalizeBehavior,
    Appsco.HeadersMixin
], PolymerElement) {
  static get template() {
...
`;
  ...
  static get is() { return 'appsco-audit-log'; }
  ...
  ready() {
      ...
      afterNextRender(this, function() {
          ...
          this._timezoneListUrl = this.resolveUrl('./data/timezone-list.json');
          this._countryListUrl = this.resolveUrl('./data/country-list.json');
      });
  }

  _computeAuditLogApi(auditLogApi, size, headers) {
      return (auditLogApi && size && headers.Authorization) ? (auditLogApi + '?limit=' + size) : null;
  }
  ...
  _formatLogInitiatorCompanies (item) {
      var initiatorCompanies = item.initiator_company_names.toString();
  ...
  _formatLogEvent(item) {
      ...
      switch (item.type) {
          ...
          case 'company_contact_created':
              ...
              var display_name = item.data.contact.email;
      ...
      if (response && response.logs) {
          ...
          if (length > 0) {
              ...
              logs.forEach(function(log, i) {
                  var record = this.formatLogItem(logs[i]);

                  var pushItem = setTimeout(function() {
  ...
  loadLog() {
      var itemPushList = this._itemPushList,
          getAuditLogRequest = this.$.ironAjaxLog;

      if (itemPushList.length > 0) {
          for (var i in itemPushList) {
  ...
  filter(filterEventType, filterFromDate, filterToDate, filterAccount) {
      var body = '';
...
window.customElements.define(AppscoAuditLog.is, AppscoAuditLog);
